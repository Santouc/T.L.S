...
- **Multiplataforma** Desktop, Edge, Mobile, Arduino
...
- **Arduino integration** para control hardware
...
- **4GB+ RAM** recomendado
- **Arduino** (opcional, para hardware integration)
...
```bash
py tf_classifier/data_capture.py
```
...
```bash
py tf_classifier/data_processor.py
```
...
```bash
py tf_classifier/train.py
```

**Resultados:**
- `tf_classifier/model.h5`
- `tf_classifier/labels.json`
- Accuracy > 90% esperado

### Paso 4: Evaluar Modelo
```bash
py tf_classifier/model_evaluator.py
```
...
│   └── imagen.py         # OpenCV utilities
├── tf_classifier/        # Sistema ML
│   ├── clasificador.py   # Clasificador principal
│   ├── train.py         # Entrenamiento
│   ├── data_capture.py  # Captura de datos
...
├── arduino_bridge.py   # Comunicación Arduino
├── deployment_integration.py # Pipeline deployment
├── arduino_sign_recognition.ino # Sketch Arduino
└── README.md           # Documentación Arduino
```
...
py deployment_integration.py
```

### Edge Deployment (TFLite)
```python
from deployment_manager import EdgeRuntime
...
### Model Parameters
```python
# En tf_classifier/train.py
model.fit(X, y, epochs=20, batch_size=32, validation_split=0.2)
```

### Confidence Threshold
```python
# En tf_classifier/clasificador.py
...
**Solución:**
- Usar TFLite: `tf_classifier/deployment_manager.py`
...
py tf_classifier/data_capture.py    # Capturar datos
py tf_classifier/data_processor.py  # Procesar datos
py tf_classifier/train.py           # Entrenar modelo
